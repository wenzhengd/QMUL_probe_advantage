...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=1.0\textwidth]{figs/fig_1_demo.pdf} 
  \caption{Cartoon schematic of two schemes of learning many-body systems. 
  (a) System-based response theory, where perturbations are applied and responses are collected directly from the target matter system ($M$).
  (b) Quantum probe-based theory, where controllable quamtum probes ($P$) are coupled to $M$ and information of $M$ is indirectly from readout of $P$. 
  The learnability of probe-based theory to reconstruct $M$'s correlators is exponentially advantageous  than system-based theory --- there are $2^{k-1}$ reconstrucable $k$-point correlators.
  }
  \label{fig:fig_1_demo}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=1.0\textwidth]{figs/fig_2_probe_circuit.pdf} 
  \caption{Circuit-model schematic of quantum probe-based learning many-body systems' correlators.  }
  \label{fig:fig_2_probe_circuit}
\end{figure}
...
[\WD{here anticommutator's involvment is clear.}]
...
\end{equation}
(\AGENT{the $g$ is NOT defined!!!!}) The induced linear change of the state is given by the \WD{Duhamel formula}
\begin{equation}
\delta\rho_A
=
-\int_0^1 d\tau\;
...
\end{equation}
(\WD{why the Gaussian modular flow ``O'' is not used?})
\AGENT{the anticommutator not appear here at all. Why?}
...
\end{equation}
with $n_{i\sigma}=c_{i\sigma}^\dagger c_{i\sigma}$. 
(\AGENT{following is redundant as free-fermions are studied already.}) Fix a subsystem $A$ (set of sites). For $U=0$ the reduced state
...
\end{align}
(\WD{minus sign is mistake in () ?})A commonly used reduced set is the connected density--density correlator on $A$,
\begin{equation}
\kappa^{(n)}_{i,j}
:=
...
\WD{Can also comment that other entanglement measures, e.g., R\'enyi entropy can also be constructed in a similar way by expanding around Gaussian reference states.}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% End of Section V
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
