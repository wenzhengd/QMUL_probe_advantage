...
\end{tabular}}&
\multirow{2}{*}{$
\mathcal{M}^{\vec{0}}(t_{[k]}) \sim [\![\![ M_{m_1}(t_1), M_{m_2}(t_2) ], \ldots , M_{m_k}(t_k)] $}
...
(open system)
\end{tabular}} &
\multirow{2}{*}{$ \mathcal{M}^{\vec{\mu}}(t_{[k]}) \sim \llbracket\cdots \llbracket M_{p_1}(t_1),M_{p_2}(t_2)\rrbracket_{\mu_1},\cdots, M_{p_k}(t_k)\rrbracket_{\mu_{k-1}} $}
& \multirow{2}{*}{$2^{k}$} &
...
The linear term in system-based response theory corresponds to leading non-linear ($k=2$) term in probe-based theory (as $M_{m_1}(t_1)$ there is not measurement basis). 
}
